(function($)
{
	...
	$('.vc_ui-button-action[data-vc-ui-element=button-save]').on('click', function()
    {
        ...
        if($('.st_add_icon_link').length >0) {
            value = $('.st_add_icon_link').find('.st-icon_link').serialize();
            $('.st-icon_link-value').val( encodeURIComponent(value) );
        }
    ...
    {
        var key = $('.st_add_icon_link').find('.icon_link-item').last().data('item');
