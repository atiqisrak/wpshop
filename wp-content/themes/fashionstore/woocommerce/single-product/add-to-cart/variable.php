...
<form class="variations_form cart" method="post" enctype='multipart/form-data' data-product_id="<?php echo absint( $product->id ); ?>" data-product_variations="<?php echo esc_attr( json_encode( $available_variations ) ) ?>">
	...
	<?php if ( empty( $available_variations ) && false !== $available_variations ) : ?>
		<p class="stock out-of-stock"><?php esc_html_e( 'This product is currently out of stock and unavailable.', 'fashionstore' ); ?></p>
	<?php else : ?>
		<?php 
		...
		<div class="wrap-attr-product <?php echo esc_attr($attribute_style)?>">
						...
						if($attribute_style == 'special'){?>
							<div class="attr-product attr-hover-box attr-<?php echo esc_attr($attribute_name)?>">
			                    ...
			                    <div class="attr-filter">
				                    <ul class="list-inline" data-attribute-id="<?php echo esc_attr($attribute_name)?>">
				                    	<?php
					                    	if ( ! empty( $options ) ) {
												if ( $product && taxonomy_exists( $attribute_name ) ) {
													...
													foreach ( $terms as $term ) {
														if ( in_array( $term->slug, $options ) ) {
															echo '<li data-attribute="' . esc_attr( $term->slug ) . '"><a href="#"><span class="bgcolor-' . esc_attr( $term->slug ) . '"></span>'.esc_html($term->name).'</a></li>';
														}
													}
												} else {
		    ...
		    <a class="reset_variations" href="#"><?php esc_html_e("Clear selection","fashionstore")?></a>
